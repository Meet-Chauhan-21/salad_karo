    ...
    <AdminLayout currentPage="memberships">
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage membership plans and user subscriptions</p>
        </div>
            ...
            {activeTab === 'plans' ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredMemberships.map((membership) => (
                  <div key={membership.id} className="bg-gray-50 rounded-lg border border-gray-200 p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center">
                        <div className={`p-2 rounded-lg ${getPlanColor(membership.planType).replace('text-', 'text-').replace('border-', 'bg-').split(' ')[0]}`}>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredUserMemberships.map((subscription) => (
                      <tr key={subscription.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getPlanColor(subscription.planType)}`}>
          ...
          <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  onClick={handleSave}
                  className="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:ml-3 sm:w-auto sm:text-sm"
                >
                        ...
                        <div>
                          ...
                          <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                            <div 
                              ...
                              style={{width: `${(selectedItem.ordersUsed / selectedItem.maxOrders) * 100}%`}}
                            ></div>
                          </div>
